...
const evaluateWithProvider = async (
  provider: "openai" | "anthropic" | "google",
  model: string,
  tool: ToolInput,
  scenario: string
...
export const optimizer = async (
  ...
  try {
    scenarios = await generateTestScenarios(
      toolDefinition,
      currentProvider,
      currentModel
    );
    ...
    console.error(
      "Failed to generate scenarios with primary provider, trying alternatives..."
    );
    ...
    for (let i = 1; i < availableProviders.length; i++) {
      const nextProviderIndex =
        (currentProviderIndex + i) % availableProviders.length;
      ...
      try {
        ...
        console.log(
          "Generated scenarios with alternative provider:",
          scenarios
        );
        break;
      } catch (nextError) {
        console.log(
          `Failed to generate scenarios with ${nextProvider} as well`
        );
      }
    }
    ...
    if (!scenariosGenerated) {
      ...
      console.log(
        "Using default scenarios as all providers failed:",
        scenarios
      );
    }
  }
  ...
  while (improvementsMade < maxImprovements) {
      ...
      try {
        ...
        for (let i = 1; i < availableProviders.length; i++) {
          ...
          try {
            ...
            console.log(
              `Alternative provider ${nextProvider} also failed:`,
              nextError.message
            );
          }
        }
    ...
    try {
      ...
      for (let i = 1; i < availableProviders.length; i++) {
        ...
        try {
          console.log(
            `Trying to improve with alternative provider: ${nextProvider}`
          );
          const improvedDescription = await improveDescription(
            nextProvider,
            nextModel,
          ...
          console.log(
            `Alternative provider ${nextProvider} also failed for improvement:`,
            nextError.message
          );
        }
      }
      ...
      if (!descriptionImproved) {
        console.log(
          "All providers failed to improve description. Returning current version."
        );
        break;
      }
    }
