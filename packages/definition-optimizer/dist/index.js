#!/usr/bin/env node
import { checkApiKeys, validateToolDefinitions, generateHash, } from "./utils.js";
 ...
 */
const loadFromCache = (hash, target) => {
 ...
 */
const saveToCache = (hash, target, optimizedDef) => {
    const cacheFile = path.join(CACHE_DIR, `${hash}-${target}.json`);
    try {
...
export const optimizeToolDefinition = async (toolDefinitions, target, optimization, model) => {
    let optimizedToolDefinitions = {
        tools: [],
    };
    ...
    const hash = generateHash(JSON.stringify(toolDefinitions) + target + optimization + (model || ""));
